...
from utils import get_bundle_path, get_net_if_mac_addresses

# MAC address public key used to verify RSA-PSS signatures in connector configs.
# This file is bundled into the .exe at build time via pytun.spec: datas=[('mac_address_pub_key', '.')].
# At runtime PyInstaller extracts it to sys._MEIPASS (./tmp/_MEIxxx/mac_address_pub_key).
# In dev mode (plain Python), it is read from the current working directory.
#
# CRITICAL — STAGING vs PRODUCTION:
# The staging build (ci.invgate.com/job/pytun-build-staging/) and the production build
# (ci.invgate.com/job/pytun-build/) each embed a DIFFERENT mac_address_pub_key.
# Connector configs generated by the staging backend are signed with the staging private key,
# and configs generated by the production backend are signed with the production private key.
# If you test a staging connector config against a production .exe (or vice versa), the MAC
# address signature will NOT match and the connector will be considered unauthorized — with NO
# informative error, since is_mac_address_signature_valid() silently returns False on any
# verification failure (device.py, except block in is_mac_address_signature_valid).
# Always use a staging-built .exe when testing staging connector configurations.
